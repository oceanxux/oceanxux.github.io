...
cd /root/lnmp2.0-full && LNMP_Auto="y" DBSelect="3" Bin="y" DB_Root_Password="password" InstallInnodb="y" PHPSelect="10" SelectMalloc="1" CheckMirror="n" ./install.sh lnmp
```
...
Create database and MySQL user with same name (y/n) y # 是否添加用户数据库
Enter current root password of Database (Password will not shown):  ##数据库root 的密码。默认是不显示
