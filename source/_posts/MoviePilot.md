...
```markdown
1. 全量同步: 用于第一次同步网盘数据（不推荐定期运行全量同步）
2. 增量同步: 用于处理离线下载进入网盘的数据，这些数据生活事件和MP事件监控都无法识别，使用全量又太过于浪费（类似于这种需求的文件都可以用增量同步）
3. MP整理事件监控: 最推荐的使用方式，配合网盘整理和MP整理上传可以完整实现自动化，无需其他过多配置
4. 生活事件监控: 只推荐开启网盘删除同步删除本地文件，其他事件监控不推荐使用
5. 网盘整理: 对于所有分享转存资源推荐必须转存到目录整理交给MP整理后再入库
```

- 我设置的MP整理事件监控: 配合网盘整理和MP整理上传可以完整实现自动化，无需其他过多配置
